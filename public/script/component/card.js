...
export default class Card extends H12 {
    ...
    async render() {
        ...
        const { name, description, onclick, icon } = this.args;

        return <>
            <div class="flex flex-row space-x-2 cursor-pointer select-none" onclick={ onclick }>
                    <div class="w-20 h-20 bg-zinc-700 rounded-lg bg-cover bg-no-repeat bg-center" style={ `background-image: url(${icon})` }></div>
